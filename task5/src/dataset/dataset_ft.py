...
class FinetuneDataset(Dataset):
    def __init__(self, args, data_path, test_model = False):
        ...
        self.stance2label = {'Lit-News_mentions':0,'non-personal_reports':1,'Self_reports':2}
        self.label2stance = {"0":'Lit-News_mentions',"1":'non-personal_reports',"2":'Self_reports'}
        
    def __getitem__(self,index):
        if self.test_mode:
            text  = self.data['tweet_text'].iloc[index]
...
#             claim = self.data['Claim'].iloc[index]
        ...
        label_t = self.data['label'].iloc[index]
        label = self.stance2label[label_t]
