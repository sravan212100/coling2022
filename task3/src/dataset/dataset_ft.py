...
class FinetuneDataset(Dataset):
    def __init__(self, args, data_path, test_model = False):
        ...
        self.stance2label = {0:0,1:1}
        self.label2stance = {0:0,1:1}
        self.web = args.web
    ...
    def __getitem__(self,index):
        if self.test_mode:
            if self.args.web:
#                 source_file = self.data['SOURCE_FILE'].iloc[index]
                drug = self.data['DRUG'].iloc[index]
#                 eou = self.data['Ease of Use'].iloc[index]
#                 eff = self.data['Effectiveness'].iloc[index]
#                 sat = self.data['Satisfaction'].iloc[index]
                text = self.data['TEXT'].iloc[index]
                input_data = self.tokenizer(drug,text,max_length=self.bert_seq_length, \
                                            padding='max_length', \
                                            truncation='longest_first')
                
            else:
#                 tid = self.data['tweet_id'].iloc[index]
#                 uid = self.data['user_id'].iloc[index]
                text = self.data['text'].iloc[index]
#                 created_at = self.data['created_at'].iloc[index]
